...
export const findAllByUser = async ( channel: ChannelType, credentials: AuthCredentials) => {
  const response = await getRequest<AdoptionApplicationResponse>(FIND_ALL_ADOPTION_PLANS, {
    headers: {
      "X-Channel": channel,
...
export const create = async (params: CreateAdoptioApplication , channel: ChannelType, credentials: AuthCredentials) => {
  const response = await postRequest<CreateAdoptioApplication>(CREATE_ADOPTION_APPLICATION, params, {
    headers: {
      "X-Channel": channel,
      "Authorization": `Bearer ${credentials.accessToken}`,
    },
